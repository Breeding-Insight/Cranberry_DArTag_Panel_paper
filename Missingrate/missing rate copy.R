library(tidyverse)
setwd("~/Desktop/Upoad github for cranberry/Missingrate")
...
F1 <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. macrocarpon (F1; 2x)")$AccessionID_changed)]
div_diplo <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. macrocarpon (diverse; 2x)")$AccessionID_changed)]
mac_4x <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. macrocarpon (4x)")$AccessionID_changed)]
mic_2x <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. microcarpum (2x)")$AccessionID_changed)]
oxy_4x <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. oxycoccos (4x)")$AccessionID_changed)]
hybrid_1 <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. meridionale × V. macrocarpon (2x)")$AccessionID_changed)]
hybrid_2 <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. oxycoccos × V. corymbosum (2x)" )$AccessionID_changed)]
blueberry_1 <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. corymbosum (4x)" )$AccessionID_changed)]
blueberry_2 <- d_sum[,which(colnames(d_sum) %in% subset(passport,passport$Species2=="V. meridionale (4x)")$AccessionID_changed)]

d_sum_all <- cbind.data.frame(F1,div_diplo,mac_4x,mic_2x,oxy_4x,hybrid_1,hybrid_2,blueberry_1,blueberry_2)
...
mr_all$Species <- rep(c("All samples", "V. macrocarpon (F1; 2x)","V. macrocarpon (diverse; 2x)","V. macrocarpon (4x)","V. microcarpum (2x)","V. oxycoccos (4x)","V. meridionale × V. macrocarpon (2x)","V. oxycoccos × V. corymbosum (2x)","V. corymbosum (4x)","V. meridionale (4x)"),
                         c(372,136,183,4,11,18,13,5,1,1))
mr_all$Species <- factor(mr_all$Species,levels = c("All samples", "V. macrocarpon (F1; 2x)","V. macrocarpon (diverse; 2x)","V. macrocarpon (4x)","V. microcarpum (2x)","V. oxycoccos (4x)","V. meridionale × V. macrocarpon (2x)","V. oxycoccos × V. corymbosum (2x)","V. corymbosum (4x)","V. meridionale (4x)"))
...
mr_all[which(grepl(pattern = "blueberry_1",mr_all$ID)),]$label <- "Blueberry_01"
mr_all[which(grepl(pattern = "blueberry_2",mr_all$ID)),]$label <- "Blueberry_02"
